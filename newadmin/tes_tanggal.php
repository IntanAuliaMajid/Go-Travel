...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_order'])) {
    ...
    $tanggal_keberangkatan_raw = $_POST['tanggal_keberangkatan'];

    // Validasi format tanggal di sisi server (berdasarkan hasil tes kita, ini akan berhasil)
    $date_obj = DateTime::createFromFormat('Y-m-d', $tanggal_keberangkatan_raw);
    if (!$date_obj || $date_obj->format('Y-m-d') !== $tanggal_keberangkatan_raw) {
        $_SESSION['error_message'] = "Format tanggal keberangkatan tidak valid. Data yang diterima: '" . htmlspecialchars($tanggal_keberangkatan_raw) . "'";
        header("Location: order_edit.php?id=" . $id_pemesanan);
        exit();
    }
    $tanggal_keberangkatan = $tanggal_keberangkatan_raw;
    ...
    $stmt_update->bind_param("sssiisdsi",
        $nama_lengkap, $email, $no_telepon, $id_paket_wisata,
...
<body>
    ...
    <main>
        <div class="page-header">
            ...
            <a href="manajemen_pemesanan.php" class="btn-back"><i class="fas fa-arrow-left"></i> Kembali</a>
        </div>
