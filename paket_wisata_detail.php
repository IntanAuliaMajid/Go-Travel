...
// Query utama untuk mengambil semua detail paket
...
// Logika untuk mengambil detail penginapan, kuliner, rencana perjalanan, dan rincian biaya (tetap sama seperti sebelumnya)
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
    ...
    .detail-hero {
        background: linear-gradient(rgba(0, 0, 0, 0.6), rgba(0, 0, 0, 0.6)), url('<?php
            ...
            // REPAIRED LOGIC:
            // 1. Try to get a package-specific image, prioritizing the thumbnail.
            ...
            // 2. If no package image, fall back to the main attraction's image.
                ...
                if ($img_wisata_result && mysqli_num_rows($img_wisata_result) > 0) {
                    ...
                    // The path in `gambar` table is like '../uploads/wisata/img.jpg', remove '../'
  ...
  <main class="detail-container">
    <div class="detail-main">
      ...
      <div class="detail-info">
          <div class="detail-meta"><h2><?php echo htmlspecialchars($wisataresult['nama_paket']); ?></h2><p><i class="fas fa-clock"></i> Durasi: <?php echo htmlspecialchars($wisataresult['durasi_paket']); ?></p></div>
          <h4 class="sidebar-subtitle" style="margin-bottom:8px">Rincian Fasilitas & Biaya:</h4>
          <?php if (!empty($termasuk_paket_list)): ?><ul style="margin:0; padding:0; list-style: none; font-size: 0.9rem;"><?php foreach($termasuk_paket_list as $item_tp): ?><li class="itemized-list-item" style="display: flex; justify-content: space-between; margin-bottom:8px;"><span class="description" style="display: inline-flex; align-items: center; gap: 8px;"><i class="fas fa-check" style="color: #2c7a51;"></i> <?php echo htmlspecialchars($item_tp['termasuk']); ?></span><?php if (isset($item_tp['harga_komponen']) && floatval($item_tp['harga_komponen']) > 0): ?><span class="price-itemized"><?php echo format_rupiah($item_tp['harga_komponen']); ?></span><?php else: ?><span class="price-itemized" style="color:#777;">Termasuk</span><?php endif; ?></li><?php endforeach; ?><?php if ($subtotal_rincian_komponen > 0) : ?><li class="itemized-list-item" style="display: flex; justify-content: space-between; margin-top: 0.8rem; padding-top: 0.8rem; border-top: 1px dashed #ddd; font-weight:bold;"><span class="description" style="color:#333;">Estimasi Subtotal</span><span class="price-itemized" style="color:#333;"><?php echo format_rupiah($subtotal_rincian_komponen); ?></span></li><?php endif; ?></ul><?php else: ?><p style="font-size: 0.9rem; color:#777;">Informasi rincian fasilitas tidak tersedia.</p><?php endif; ?>
          <div class="price-display-container"><span class="price-display-label">Harga Paket Mulai:</span><div class="price-display-value"><span class="price-display-amount"><?php echo format_rupiah($harga_final_untuk_sidebar); ?></span><span class="price-display-per-person">per orang</span></div></div>
          <h4 class="sidebar-subtitle" style="margin-top:1.5rem; margin-bottom:0.5rem;">Info Penting:</h4><ul class="info-penting-list" style="list-style: none; padding: 0; font-size: 0.9rem;"><?php if (!empty($wisataresult['info_penting'])): ?><li style="display:flex; gap:8px; align-items:flex-start;"><i class="fas fa-info-circle" style="color:#2c7a51; margin-top:4px;"></i> <span><?php echo nl2br(htmlspecialchars($wisataresult['info_penting'])); ?></span></li><?php else: ?><li style="display:flex; gap:8px; align-items:flex-start;"><i class="fas fa-info-circle" style="color:#2c7a51; margin-top:4px;"></i> <span>Tidak ada info penting tambahan untuk paket ini.</span></li><?php endif; ?></ul>
          <a href="pemesanan.php?id_paket=<?php echo htmlspecialchars($id_paket_wisata); ?>" class="pesan-sekarang-link"><button class="pesan-sekarang"><i class="fas fa-shopping-cart"></i> Pesan Paket Sekarang</button></a>
      </div>
    </div>
  </main>
